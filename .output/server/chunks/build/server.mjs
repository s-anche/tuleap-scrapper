import { defineComponent as defineComponent$1, shallowRef, getCurrentInstance as getCurrentInstance$1, provide, cloneVNode, h, createElementBlock, ref, computed, hasInjectionContext, inject as inject$1, createVNode, Transition, mergeProps, toRefs, watch, nextTick, toRef, watchEffect, normalizeStyle, normalizeClass, createElementVNode, useId, onScopeDispose, withDirectives, vShow, Fragment, toDisplayString, reactive, toRaw, createTextVNode, capitalize, readonly, defineAsyncComponent, unref, shallowReactive, Suspense, useSSRContext, createApp, TransitionGroup, camelize, isRef, toValue, resolveDynamicComponent, Text, markRaw, resolveComponent, render, withCtx, onErrorCaptured, onServerPrefetch, effectScope, isReadonly, isShallow, isReactive, warn, getCurrentScope, vModelText, Teleport, withModifiers, isVNode, Comment, normalizeProps, guardReactiveProps } from 'vue';
import { i as hasProtocol, k as isScriptProtocol, l as joinURL, w as withQuery, s as sanitizeStatusCode, m as getContext, $ as $fetch$1, n as createHooks, o as executeAsync, c as createError$1, t as toRouteMatcher, p as createRouter$1, q as defu } from '../nitro/nitro.mjs';
import { b as baseURL$1 } from '../routes/renderer.mjs';
...
const _routes = [
  {
    ...
    component: () => import('./index-BmpD35Nr.mjs')
  },
  {
    name: "login",
    path: "/login",
    meta: { ...__nuxt_page_meta$2 || {}, ...{ "middleware": "guest" } },
    component: () => import('./login-QLK0TRsS.mjs')
  },
  {
    name: "epic-id",
    path: "/epic/:id()",
    meta: { ...__nuxt_page_meta$1 || {}, ...{ "middleware": "auth" } },
    component: () => import('./_id_-fqz5wXNB.mjs')
  },
  {
    name: "stories-table",
    path: "/stories-table",
    meta: { ...__nuxt_page_meta || {}, ...{ "middleware": "auth" } },
    component: () => import('./stories-table-D3wr0EIb.mjs')
  }
];
...
const namedMiddleware = {
  auth: () => import('./auth-68TxKp5P.mjs'),
  guest: () => import('./guest-B0-ihsOD.mjs')
};
...
function keyboardRippleShow(e, keys2) {
  if (!keyboardRipple && keys2.includes(e.keyCode)) {
    keyboardRipple = true;
    rippleShow(e);
  }
}
...
function useValidation(props) {
  ...
  watch([isValid2, errorMessages], () => {
    form.update?.(uid.value, isValid2.value, errorMessages.value);
  });
  ...
  async function handleDrop(e) {
    ...
    if (entries.length) {
      for (const entry2 of entries) {
        const files = await traverseFileTree(entry2, appendIfDirectory(".", entry2));
  ...
  return new Promise((resolve, reject) => {
    if (item.isFile) {
      ...
      directoryReader.readEntries(async (entries) => {
        const files = [];
        for (const entry2 of entries) {
          files.push(...await traverseFileTree(entry2, appendIfDirectory(path, entry2)));
        }
        resolve(files);
      });
    }
...
function useHold(_ref) {
  ...
  function holdStop() {
    ...
    (void 0).removeEventListener("pointerup", holdStop);
    (void 0).removeEventListener("blur", holdStop);
  }
  function tick(value) {
...
const VTimePickerClock = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    function onMouseDown(e) {
      if (props.disabled) return;
      e.preventDefault();
      (void 0).addEventListener("mousemove", onDragMove);
      (void 0).addEventListener("touchmove", onDragMove);
      (void 0).addEventListener("mouseup", onMouseUp);
      (void 0).addEventListener("touchend", onMouseUp);
    ...
    function onMouseUp(e) {
      e.stopPropagation();
      (void 0).removeEventListener("mousemove", onDragMove);
      (void 0).removeEventListener("touchmove", onDragMove);
      (void 0).removeEventListener("mouseup", onMouseUp);
      (void 0).removeEventListener("touchend", onMouseUp);
      isDragging.value = false;
...
const layouts = {
  auth: defineAsyncComponent(() => import('./auth-30ieurar.mjs').then((m) => m.default || m)),
  default: defineAsyncComponent(() => import('./default-BoElCFpL.mjs').then((m) => m.default || m))
};
...
const sprintCache = /* @__PURE__ */ new Map();
const apiService = {
  // Projects
  ...
  // Trackers
  async getProjectTrackers(projectId, params = {}) {
  ...
  async getTracker(id) {
    return await apiRequest(`/trackers/${id}`);
  },
  // Artifacts
  async getTrackerArtifacts(trackerId, params = {}) {
    const response = await apiRequest(
  ...
  async getArtifact(id, params = {}) {
    return await apiRequest(`/artifacts/${id}`, {
      method: "GET",
      query: params
    });
  },
  async getArtifacts(ids, params = {}) {
    const promises = ids.map(
      (id) => apiRequest(`/artifacts/${id}`, {
        method: "GET",
        query: params
      })
    );
  ...
  buildTuleapUrl(htmlUrl, artifactId) {
  ...
  async getEpicById(id) {
    try {
      const artifact = await apiRequest(`/artifacts/${id}`);
  ...
  transformArtifactToEpic(artifact) {
    const getFieldValue = (fieldId) => {
      const field = artifact.values?.find((v) => v.field_id === fieldId);
      return field?.value || field?.values?.[0]?.label || null;
    };
    const getFieldByLabel = (label) => {
      const field = artifact.values?.find((v) => v.label === label);
      return field?.value || field?.values?.[0]?.label || null;
    };
    const getLinksField = () => {
      ...
      for (const fieldName of possibleLinkFields) {
        const field = artifact.values?.find((v) => v.label === fieldName);
        if (field) {
          return field?.links || [];
        }
      }
    ...
    const getTagsFromField = () => {
      const field = artifact.values?.find((v) => v.label === "Tags");
    ...
    return {
      ...
      summary: this.decodeHtmlEntities(
        artifact.values_by_field?.description?.value || artifact.values_by_field?.description?.commonmark || getFieldByLabel("Description") || ""
      ),
  ...
  extractFieldValue(artifact, fieldName) {
    const field = artifact.values?.find((v) => v.label === fieldName);
    return field?.value || field?.values?.[0]?.label || null;
  },
  extractPoints(artifact) {
    const points = this.extractFieldValue(artifact, "Points") || this.extractFieldValue(artifact, "Story Points") || this.extractFieldValue(artifact, "Estimation") || artifact.values_by_field?.points?.value || artifact.values_by_field?.story_points?.value || artifact.values_by_field?.estimation?.value;
    if (points === void 0 || points === null || points === "") {
      return null;
    }
    const numericPoints = Number(points);
    return isNaN(numericPoints) ? null : numericPoints;
  },
  extractRemainingEffort(artifact) {
    const remainingEffort = this.extractFieldValue(artifact, "Remaining Effort") || this.extractFieldValue(artifact, "Remaining effort") || artifact.values_by_field?.remaining_effort?.value;
    return remainingEffort ? Number(remainingEffort) : null;
  },
  async extractSprintInfo(artifact) {
    const directSprint = this.extractFieldValue(artifact, "Sprint") || this.extractFieldValue(artifact, "Iteration") || artifact.values_by_field?.sprint?.value || artifact.values_by_field?.iteration?.value;
    if (directSprint) {
      return String(directSprint);
    }
    const linksField = artifact.values?.find((v) => v.label === "Links");
    const reverseLinks = linksField?.reverse_links || [];
    const sprintLink = reverseLinks.find(
      (link) => link.tracker.label.toLowerCase().includes("sprint")
    );
    if (sprintLink) {
      try {
  ...
  async getLinkedArtifacts(artifactId, direction = "forward", nature = "_is_child") {
    try {
      const response = await apiRequest(
  ...
  async getEpicTreeData(epic) {
    ...
    const features = await Promise.all(
      relatedArtifacts.features.map(async (feature) => {
        ...
        const percentWithPoints = linkedArtifactCount > 0 ? Math.round(artifactsWithPoints.length / linkedArtifactCount * 100) : 0;
        return {
          artifact: feature,
          subArtifacts,
          points,
...
const useEpicStore = defineStore("epics", () => {
  ...
  const epicIdsList = computed(() => {
    return epicIds.value.split(",").map((id) => id.trim()).filter((id) => id && /^\d+$/.test(id)).map((id) => parseInt(id, 10));
  });
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    const stack = void 0;
    const _Error404 = defineAsyncComponent(() => import('./error-404-t0gS6ilw.mjs'));
    const _Error = defineAsyncComponent(() => import('./error-500-De6uG83z.mjs'));
...
export { __nuxt_component_0$1 as _, useNuxtApp as a, useRuntimeConfig as b, nuxtLinkDefaults as c, useEpicStore as d, entry$1 as default, useAuthStore as e, useRoute$1 as f, apiService as g, defineNuxtRouteMiddleware as h, navigateTo as n, resolveRouteObject as r, useRouter$1 as u };
